...
def run_epoch(epoch, model, optimizer, loader, loss_computer, logger, csv_logger, args,
              is_training, show_progress=False, log_every=50, scheduler=None, total_weights=None):
    """
    ...
    if args.reweight_en == True:
        with torch.no_grad():
            ...
            for batch in dataset['train_loader']:
                ...
                segment_ids = batch[0][:, :, 2].cuda()
                labels = batch[1].cuda()

                logits = model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=segment_ids, labels=labels)[1]
            ...
            total_weights = compute_weights1(all_train_logits, all_train_y_true, args.gamma, True)
